...
#if 0//<Coverity : EVALUATION_ORDER>
...
#if 1//<Coverity : EVALUATION_ORDER>
inline uchar get_uchar(gunzip_t *guz)
{
	return (uchar)(guz->inptr < guz->insize ? guz->inbuf[guz->inptr++] : 0);
}
inline ushort get_ushort(gunzip_t *guz)
{
	return ((ushort)get_uchar(guz) | (ushort)get_uchar(guz) << 8);
}
inline ulong get_ulong(gunzip_t *guz)
{
	return ((ulong)get_uchar(guz)|(ulong)get_uchar(guz)<<8|(ulong)get_uchar(guz)<<16|(ulong)get_uchar(guz) << 24);
}
#endif
