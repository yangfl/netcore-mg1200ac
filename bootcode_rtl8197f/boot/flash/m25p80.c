...
int spi_flw_image(unsigned int chip, unsigned int flash_addr_offset ,unsigned char *image_addr, unsigned int image_size)
{
	...
	if(flash_addr_offset+image_size>=g_m25p.mtd.size)
	{
		printf("Error: Write over flash size 0x%08x\n", (long )g_m25p.mtd.size);
		return 0;
	}
...
{
	//set_baudrate(&g_m25p, baudrate_divider_ocp);
	spi_set_baudrate(g_m25p.spi, baudrate_divider_ocp);
	return;
}

void flash_set_dummy_cycle_clb(uint32_t dum_cycle, uint32_t tunning_dummy)
{
	spi_set_dummy_cycle_clb(g_m25p.spi, dum_cycle, tunning_dummy);
	return;
}
...
{
	return spi_get_clb_startaddr(g_m25p.spi);
}

void flash_set_dummy_back()
{
	spi_set_dummy_back(g_m25p.spi);
}
...
int rtk_get_flashInfo(int type,void* value)
{
	switch(type){
		case ERASE_SIZE:
			*(int*)value = g_m25p.mtd.erasesize;
			break;
		case FLASH_SIZE:
			*(int*)value = g_m25p.mtd.size;
			break;
		default:
