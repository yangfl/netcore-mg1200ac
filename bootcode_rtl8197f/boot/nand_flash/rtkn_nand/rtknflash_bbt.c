...
{
	struct nand_chip* this = (struct nand_chip*)((struct mtd_info*)mtd->priv);
	struct rtknflash *rtkn = (struct rtknflash *)this->priv;
...
static int rtk_nand_scan_bbt(struct mtd_info *mtd)
{
	nand_printf("[%s]:%d\n",__func__,__LINE__);
	struct nand_chip *this = (struct nand_chip *)mtd->priv;
	struct rtknflash *rtkn = (struct rtknflash *)((struct nand_chip *)this->priv);
	int isLastPage = rtkn->chip_param.isLastPage;
	int rc = 0, i;
	__u8 isbbt;
	u8 *temp_BBT=0;
...
int rtk_remapBBT_write_fail(struct mtd_info* mtd,unsigned int page,const uint8_t *buf,int oob_required)
{
	...
	for ( i=0; i<backup_offset; i++){
		int ret = 0;
		ret = rtk_scan_read_oob(mtd,rtkn->tmpBuf,(block*ppb+i),page_size);
		ret = rtk_scan_write_bbt(mtd,(block_remap*ppb+i),page_size,rtkn->tmpBuf,&rtkn->tmpBuf[page_size]);
	}
	...
	rtk_scan_write_bbt(mtd,(block_remap*ppb+backup_offset),page_size,(uint8_t *)buf,&rtkn->tmpBuf[page_size]);		
...
#ifdef CONFIG_RTK_NORMAL_BBT
	rtkn->BBT_PERCENT = 5;
	bbt_num = rtkn->bbt_num = (this->chipsize >> this->phys_erase_shift) * rtkn->BBT_PERCENT/100;
