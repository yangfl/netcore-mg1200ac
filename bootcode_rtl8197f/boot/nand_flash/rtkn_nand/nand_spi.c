...
SECTION_NAND_SPI void
winbond_2KB_ecc_encode_buffered(void *dma_addr,
                                        void *des_addr,
                                        void *p_eccbuf,
...
)
{
    ...
    ecc_encode_bch_6t((void *)des_addr, p_eccbuf,pagenum);
    //winbond_2KB_chunk_write(temp_page_buffer, blk_pge_addr);
}

#if CONF_SPI_NAND_UBOOT_COMMAND
static int
winbond_2KB_chunk_ecc_encode_continue(cmd_tbl_t * cmdtp, int flag,
                                              int argc, char *argv[]
)
{
   ...
   if ((argv[1]) && (argv[2]) && (argv[3]) ) {
    ...
    {
        winbond_2KB_ecc_encode_buffered((void*)src_addr, (void*)des_addr,
                                            p_eccbuf,i);
	//blk_pge_addr++;
...
U_BOOT_CMD(snwbeccgen, 5, 0, winbond_2KB_chunk_ecc_encode_continue,
