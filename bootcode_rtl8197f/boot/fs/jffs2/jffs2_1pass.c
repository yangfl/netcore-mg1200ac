...
static int read_nand_cached(u32 off, u32 size, u_char *buf)
{
	...
	while (bytes_read < size) {
		if ((off + bytes_read < nand_cache_off) ||
...
#else
				nand_cache = (u8*)malloc(NAND_CACHE_SIZE);
#endif
				if (!nand_cache) {
			...
			if (nand_read(&nand_info[id->num], nand_cache_off,
					...
					retlen != NAND_CACHE_SIZE) 
#else
			if(nflashread((unsigned long)nand_cache,(unsigned long)nand_cache_off,retlen,0))
#endif			
			{
	...
	return bytes_read;
#else
	if(rtkn_read_jffs2((unsigned long)buf,(unsigned long)off,size))
	{
		printf("read_nand_cached: error reading nand off %#x size %d bytes\n",
				nand_cache_off, NAND_CACHE_SIZE);
		return -1;
...
#else
		memBase = (struct mem_block *)mmalloc(sizeof(struct mem_block));		
#endif
		if (memBase == NULL) {
...
#else
	if (NULL != (part->jffs2_priv = (void*)malloc(sizeof(struct b_lists)))) {
#endif
		pL = (struct b_lists *)part->jffs2_priv;
...
#else
	printf(" %s %x %s", mkmodestr(st->st_mode,str), (u32)st->st_size,  name);
#endif
}
...
#else	
	pL->readbuf = (void*)malloc(max_totlen);
#endif
