...
#ifdef __RTK_BOOT__
static struct part_info rtk_part;
#endif
...
#else
int do_jffs2_fsload(int argc, char * const argv[])
#endif
{
	char *fsname;
	char *filename;
...
#else
	struct part_info *part = &rtk_part;
	ulong offset;
#endif
...
#ifndef __RTK_BOOT__
		printf("### %s loading '%s' to 0x%1x\n", fsname, filename, offset);
#else
		printf("### %s loading '%s' to %x\n", fsname, filename, offset);
#endif
...
#else
			printf("### %s load complete: %d bytes loaded to %x\n",
				fsname, size, offset);
#endif
		} else {
...
#else
int do_jffs2_ls(int argc, char * const argv[])
#endif
{
	char *filename = "/";
	int ret;
...
#else
int do_jffs2_fsinfo(int argc, char * const argv[])
#endif
{
#ifndef __RTK_BOOT__
	struct part_info *part;
